...
def test_get_artist():
    assert spotify_service.get_artist("Taylor Swift") == "spotify:artist:06HL4z0CvFAxyc27GXpf02"

@pytest.mark.skipif(os.getenv("CI")=="true", reason="avoid issuing on the CI server") # skips this test on CI
def test_verifyartist():
    artist = spotify_service.verify_artist("spotify:artist:06HL4z0CvFAxyc27GXpf02") #artist will look like "Taylor Swift"
    assert isinstance(artist,str)

@pytest.mark.skipif(os.getenv("CI")=="true", reason="avoid issuing on the CI server") # skips this test on CI
def test_artist_albums():
    results = spotify_service.artist_albums("spotify:artist:06HL4z0CvFAxyc27GXpf02")
...
def test_artist_top_tracks():
    results = spotify_service.artist_top_tracks("spotify:artist:06HL4z0CvFAxyc27GXpf02")
    assert isinstance(results,list)
...
def test_artist_song_recs():
    results = spotify_service.artist_song_recommendations("spotify:artist:06HL4z0CvFAxyc27GXpf02")
    assert isinstance(results,list)
...
def test_artist_recs():
    results = spotify_service.artist_recs("spotify:artist:06HL4z0CvFAxyc27GXpf02")
    assert isinstance(results,list)
...
def test_get_characteristics():
    ...
    results = spotify_service.get_characteristics(id)
